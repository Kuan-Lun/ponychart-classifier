"""Probability cache：跨 session 持久化模型預測結果與 thresholds。
...
def load(cache_path: Path, model_path: Path) -> CacheData | None:
    ...
    try:
        ...
        if not isinstance(probs, dict) or not isinstance(thresholds, list):
            return None
        return CacheData(probs=probs, thresholds=thresholds)
    except Exception:
        return None
...
def save(
    ...
    """將 probs、thresholds 與當前 model hash 寫入 cache（atomic write）。"""
    data = {
