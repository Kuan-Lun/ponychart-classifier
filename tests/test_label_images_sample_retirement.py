...
from app.label_images.retirement_ledger import RetirementReceiptLedger
...
def _ledger(image_dir: Path) -> RetirementReceiptLedger:
    return RetirementReceiptLedger(image_dir / "retirement_receipts.json")
...
def test_trigger_uses_strict_second_precision_and_canonical_original(
    ...
    assert plan_source_save(equal, [1], store, ledger) is None
    assert plan_source_save(after, [], store, ledger) is None
    assert plan_source_save(crop, [1], store, ledger) is None
    assert plan_source_save(image_dir / "not-a-pony.png", [1], store, ledger) is None
    assert plan_source_save(after, [1], store, ledger) == SourceSavePlan(
        after.stem, retire_oldest=True
    )


def test_crop_label_does_not_trigger_or_mark_original_as_completed(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    image_dir = tmp_path / "rawimage"
    ...
    store = _store(image_dir, monkeypatch)
    ledger = _ledger(image_dir)
    source = "pony_chart_20260825_000001_000000_after001"
    original = image_dir / f"{source}.png"
    crop = image_dir / f"{source}_crop1.png"

    store.set(store.path_to_key(crop), [2])
    assert plan_source_save(crop, [2], store, ledger) is None
    assert plan_source_save(original, [2], store, ledger) == SourceSavePlan(
        source, retire_oldest=True
    )

    store.set(store.path_to_key(original), [2])
    assert plan_source_save(original, [3], store, ledger) == SourceSavePlan(
        source, retire_oldest=False
    )
    ledger.record(source)
    ledger.save()
    assert plan_source_save(original, [3], store, ledger) is None


def test_loaded_empty_original_label_is_not_a_completed_source_save(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    image_dir = tmp_path / "rawimage"
    ...
    assert store.all_keys() == [original.name]
    assert plan_source_save(original, [1], store, ledger) == SourceSavePlan(
        source,
        retire_oldest=True,
    )
...
def test_loaded_empty_original_label_is_not_a_retirement_candidate(
    ...
    retired = save_and_retire_oldest_sample(
        ...
        store,
        ledger,
        SourceSavePlan(new_source, retire_oldest=True),
    )
    ...
    assert ledger.contains(new_source)


def test_malformed_receipt_ledger_fails_closed_without_planning_retirement(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    image_dir = tmp_path / "rawimage"
    image_dir.mkdir()
    store = _store(image_dir, monkeypatch)
    ledger_path = image_dir / "retirement_receipts.json"
    ledger_path.write_text("{broken", encoding="utf-8")
    ledger = RetirementReceiptLedger(ledger_path)
    new = image_dir / "pony_chart_20260825_000001_000000_new00001.png"

    assert ledger.usable is False
    assert plan_source_save(new, [1], store, ledger) is None
...
def test_retirement_rolls_back_files_and_labels_when_commit_fails(
    ...
    assert not ledger.contains(new_source)
    assert (image_dir / "labels.json").read_bytes() == before
    assert list(tmp_path.glob(".rawimage-retirement-*")) == []


def test_retirement_rolls_back_labels_files_and_receipt_when_ledger_save_fails(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    image_dir = tmp_path / "rawimage"
    ...
    store.set(store.path_to_key(new), [2])
    ledger = _ledger(image_dir)
    prior_source = "pony_chart_20260826_000000_000000_prior001"
    ledger.record(prior_source)
    ledger.save()
    ledger_before = (image_dir / "retirement_receipts.json").read_bytes()
    ...
    assert old.exists()
    assert new.exists()
    assert store.get(store.path_to_key(old)) == [1]
    assert store.get(store.path_to_key(new)) == [2]
    assert (image_dir / "labels.json").read_bytes() == labels_before
    assert (image_dir / "retirement_receipts.json").read_bytes() == ledger_before
    assert ledger.contains(prior_source)
    assert not ledger.contains(new_source)
    assert list(tmp_path.glob(".rawimage-retirement-*")) == []
...
def test_startup_recovery_restores_partial_file_move_and_exact_metadata(
    ...
    assert recover_retirement_transaction(image_dir, store.file_path, ledger.file_path)
    assert old.exists()
    assert crop.exists()
    assert store.file_path.read_bytes() == labels_before
    assert not ledger.file_path.exists()
...
def test_startup_recovery_rolls_back_partial_metadata_commit(
    ...
    ledger.record(prior_source)
    ledger.save()
    ledger_before = ledger.file_path.read_bytes()
    original_ledger_save = ledger.save

    def crash_after_ledger_save() -> None:
        original_ledger_save()
        raise SystemExit("simulated process death")

    monkeypatch.setattr(ledger, "save", crash_after_ledger_save)
    with pytest.raises(SystemExit, match="simulated process death"):
        save_and_retire_oldest_sample(
            image_dir,
            store,
    ...
    assert not old.exists()
    assert store.file_path.read_bytes() != labels_before
    assert ledger.file_path.read_bytes() != ledger_before

    assert recover_retirement_transaction(image_dir, store.file_path, ledger.file_path)
    assert old.exists()
    assert store.file_path.read_bytes() == labels_before
    assert ledger.file_path.read_bytes() == ledger_before
    recovered_ledger = RetirementReceiptLedger(ledger.file_path)
    assert recovered_ledger.contains(prior_source)
    assert not recovered_ledger.contains(new_source)
...
def test_startup_recovery_keeps_committed_metadata_and_finishes_cleanup(
    ...
    committed_ledger = ledger.file_path.read_bytes()
    assert not old.exists()
    ...
    assert ledger.file_path.read_bytes() == committed_ledger
...
def test_malformed_or_inconsistent_journal_fails_closed_before_candidate_scan(
    ...
    with pytest.raises(RetirementRecoveryError, match="candidate path"):
        recover_retirement_transaction(image_dir, store.file_path, ledger.file_path)
...
class _FakeApp:
    def __init__(
        ...
        self.store = store
        self.retirement_receipts = ledger
...
def test_every_non_retirement_save_is_blocked_by_pending_journal(
    ...
    receipt_source = (
        source
        if save_kind == "repeat"
        else "pony_chart_20260826_000000_000000_prior001"
    )
    ledger.record(receipt_source)
    ledger.save()
    journal_path = journal_path_for(image_dir)
    ...
    app = _FakeApp(current, store, ledger)
...
def test_label_action_retires_only_on_first_original_save(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    image_dir = tmp_path / "rawimage"
    ...
    store.save()
    app = _FakeApp(original, store, _ledger(image_dir))
    ...
    app.current_labels = []
    actions.save()
    app.retirement_receipts = _ledger(image_dir)
    app.current_labels = [2]
    actions.save()

    assert old2.exists()
    assert store.get(store.path_to_key(original)) == [2]
    assert app.retirement_receipts.contains(source)
    assert app.messages == ["saved", "saved", "saved"]


def test_existing_labeled_original_backfills_receipt_without_retirement(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    image_dir = tmp_path / "rawimage"
    ...
    def save_and_record(
        ...
        received_ledger: RetirementReceiptLedger,
        plan: SourceSavePlan,
        *,
        ...
        plans.append(plan)
        received_store.save()
        received_ledger.record(plan.source_stem)
        received_ledger.save()

    monkeypatch.setattr(
        "app.label_images.app.save_and_retire_oldest_sample", save_and_record
    )
    ...
    assert plans == [SourceSavePlan(source, retire_oldest=False)]
    assert app.retirement_receipts.contains(source)
...
def test_label_action_crop_save_never_triggers_retirement(
    ...
    app = _FakeApp(crop, store, _ledger(image_dir))
    calls: list[str] = []
    ...
    monkeypatch.setattr(
        ...
        lambda image_dir, label_store, ledger, plan, before_retire=None: calls.append(
            plan.source_stem
        ),
    )
...
def test_label_action_with_malformed_ledger_saves_label_without_retirement(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    image_dir = tmp_path / "rawimage"
    ...
    ledger_path = image_dir / "retirement_receipts.json"
    ledger_path.write_text("{broken", encoding="utf-8")
    app = _FakeApp(original, store, RetirementReceiptLedger(ledger_path))
    ...
    monkeypatch.setattr(
        "app.label_images.app.save_and_retire_oldest_sample",
        lambda image_dir, label_store, ledger, plan: calls.append(plan),
    )

    _LabelActions(app).save()  # type: ignore[arg-type]

    assert calls == []
    assert store.get(store.path_to_key(original)) == [1]
    assert ledger_path.read_text(encoding="utf-8") == "{broken"
...
def test_label_action_syncs_analysis_cache_and_cleans_empty_directories(
    ...
    old = _image(image_dir, f"1/twilight/{old_source}.png")
    old_key = store.path_to_key(old)
    app = _FakeApp(new, store, _ledger(image_dir))
...
def test_label_action_rolls_back_new_file_and_memory_when_retirement_fails(
    ...
    def fail_retirement(
        ...
        *,
        ...
        del received_dir, received_store, received_ledger, plan, before_retire
    ...
    assert not (image_dir / "labels.json").exists()
    assert not app.retirement_receipts.contains(source)
    assert not (image_dir / "retirement_receipts.json").exists()
...
def test_label_action_retirement_keeps_current_new_image_with_real_navigator(
    ...
    later = _image(
        image_dir,
        "pony_chart_20260825_000002_000000_new00002.png",
    )
    store.set(store.path_to_key(old), [1])
    store.save()
    nav = ImageNavigator([old, new, later], store)
    ...
    assert nav.all_paths == [new, later]
