...
def _key_for_index(idx: int) -> str:
    return f"rawimage/pony_chart_20240101_{idx:06d}.png"
...
def test_select_recent_original_samples_break_ties_by_full_path_descending() -> None:
    samples = [
        Sample(path="/tmp/a/pony_chart_20240101_000000.png", labels=[1]),
        Sample(path="/tmp/z/pony_chart_20240101_000000.png", labels=[1]),
        Sample(path="/tmp/m/pony_chart_20231231_235959.png", labels=[1]),
    ]

    selected = select_recent_original_samples(samples)

    assert [sample.path for sample in selected] == [
        "/tmp/z/pony_chart_20240101_000000.png",
        "/tmp/a/pony_chart_20240101_000000.png",
        "/tmp/m/pony_chart_20231231_235959.png",
    ]
...
def test_prepare_balanced_samples_runs_gof_only_on_recent_subset(
    ...
    samples = [
        Sample(
            path=f"/tmp/pony_chart_20240101_{idx:06d}.png",
